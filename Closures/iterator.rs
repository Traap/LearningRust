...
pub trait Iterator {
    ...
    fn any<F>(&mut self, f: F) -> bool where
        // 'FnMut' meaning any captured variable may be at most be modified, not consumed.
...
fn main() {
    ...
    // 'iter()' for vecs yields '%i32'. De-structure to 'i32'.
    println!("2 in vec1: {}", vec1.iter()     .any(|&x| x == 2));

    // 'into_iter()' for vecs yields 'i32'.  No de-structuring required.
    ...
    // 'into_inter()' for arrays unusually yields '&i32'.
